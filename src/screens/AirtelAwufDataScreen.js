...
const DUMMY_PACKAGES = [
  { id: 'aaw1', title: 'Awuf 50MB - 1 Day', subtitle: 'Quick social browsing' },
  { id: 'aaw2', title: 'Awuf 150MB - 7 Days', subtitle: 'Light weekly usage' },
  { id: 'aaw3', title: 'Awuf 500MB - 14 Days', subtitle: 'Balanced short plan' },
  { id: 'aaw4', title: 'Awuf 1GB - 30 Days', subtitle: 'Monthly Awuf bundle' },
];
...
export default function AirtelAwufDataScreen({ user, onBack, themeMode = 'dark', onOpenDeposit, onSuccess }) {
  ...
  async function tryBiometricAuth() {
    try {
      ...
      if (res.success) {
        ...
        const payload = { success: true, provider: 'airtel_awuf', phone, selectedPackage, amount: getPayableAmount(), voucherApplied, timestamp: Date.now() };
  ...
  const AIRTEL_PREFIXES = ['0802','0808','0708','0701','0812','0902','0907','0912','0904','0818'];
  ...
  function getAmountForPackage(pkg) {
    ...
    if (title.includes('50mb')) return 30;
    if (title.includes('150mb')) return 80;
    if (title.includes('500mb')) return 180;
    if (title.includes('1gb')) return 380;
    return 120;
  }
  ...
  function applyVoucher() {
    if (!voucher) return;
    if (voucher.trim().toUpperCase() === 'AWUFAT') {
      ...
      <View style={[styles.balanceCard, { backgroundColor: palette.surface }]}> 
        ...
        <View style={styles.delRateRow}>
          <View style={[styles.delBarBackground, { backgroundColor: barBgColor }]}> 
            <View style={[styles.delBarFill, { width: '86%', backgroundColor: '#2ECC71' }]} />
          </View>
          <Text style={[styles.delLabel, { color: palette.textMuted }]}>Airtel Awuf Delivery</Text>
          <Text style={[styles.delPct, { color: palette.text }]}>86.00%</Text>
        </View>
        <Text style={[styles.dialText, { color: palette.textMuted }]}>Dial *123*5# to check Awuf balances</Text>
      </View>
      ...
      <Modal visible={authVisible} animationType="slide" transparent>
        <View style={styles.authOverlay}>
          <View style={[styles.authSheet, { backgroundColor: palette.surface }]}> 
            ...
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Product</Text><Text style={[styles.authValue, { color: palette.text }]}>{selectedPackage?.title || 'Airtel Awuf Bundle'}</Text></View>
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Recipient</Text><Text style={[styles.authValue, { color: '#E60000' }]}>{phone}</Text></View>
