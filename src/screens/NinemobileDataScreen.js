...
const DUMMY_PACKAGES = [
  ...
  { id: 'p2', title: '1GB - 7 Days', subtitle: 'Good for light browsing' },
  { id: 'p3', title: '2GB - 30 Days', subtitle: 'Balanced monthly plan' },
  { id: 'p4', title: '5GB - 30 Days', subtitle: 'For heavier usage' },
];

export default function NinemobileDataScreen({ user, onBack, themeMode = 'dark', onOpenDeposit, onSuccess }) {
  ...
  async function tryBiometricAuth() {
    try {
      ...
      if (res.success) {
        ...
        const payload = { success: true, provider: '9mobile', phone, selectedPackage, amount: getPayableAmount(), voucherApplied, timestamp: Date.now() };
  ...
  const NINE_PREFIXES = ['0809','0817','0818','0909','0908','0917'];
  ...
  function verifyNumber() {
    ...
    const isNine = NINE_PREFIXES.some((pre) => normal.startsWith(pre));
    if (isNine) {
      setVerified(true);
  ...
  function renderPackage({ item }) {
    return (
      <TouchableOpacity style={[styles.pkgRow, { backgroundColor: palette.surface }]} onPress={() => { setSelectedPackage(item); setPackagesVisible(false); }}>
          ...
          <Image source={require('../../public/9mobile-logo.png')} style={styles.pkgIcon} />
        </View>
  ...
  function getAmountForPackage(pkg) {
    ...
    if (title.includes('500mb')) return 120;
    if (title.includes('1gb')) return 220;
    if (title.includes('2gb')) return 420;
    if (title.includes('5gb')) return 950;
    return 200;
  }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: palette.background, paddingTop: safeTop }]}> 
      ...
      <View style={[styles.balanceCard, { backgroundColor: palette.surface }]}> 
        ...
        <Image source={require('../../public/9mobile-logo.png')} style={styles.mtnLogo} />
        <View style={styles.delRateRow}>
          <View style={[styles.delBarBackground, { backgroundColor: barBgColor }]}> 
            <View style={[styles.delBarFill, { width: '85%', backgroundColor: '#2ECC71' }]} />
          </View>
          <Text style={[styles.delLabel, { color: palette.textMuted }]}>9mobile Delivery Rate</Text>
          <Text style={[styles.delPct, { color: palette.text }]}>85.00%</Text>
        </View>
        <Text style={[styles.dialText, { color: palette.textMuted }]}>Dial *228# to check data balance</Text>
      </View>
      ...
      <Modal visible={authVisible} animationType="slide" transparent>
        <View style={styles.authOverlay}>
          <View style={[styles.authSheet, { backgroundColor: palette.surface }]}> 
            ...
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Product</Text><Text style={[styles.authValue, { color: palette.text }]}>{selectedPackage?.title || '9mobile Data Bundle'}</Text></View>
