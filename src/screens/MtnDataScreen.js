...
const DUMMY_PACKAGES = [
  ...
  { id: 'p2', title: '120MB - Facebook - 1 Month', subtitle: 'Dial *323*4# to check data balance' },
  { id: 'p3', title: 'MTN 1.2GB - Social Plan - 30 Days', subtitle: 'Works for all social media apps' },
  { id: 'p4', title: 'MTN 2GB - 2 Days', subtitle: 'Dial *323*4# to check data balance.' },
  { id: 'p5', title: 'MTN 2.5GB - 2 Days', subtitle: 'Dial *323*4# to check data balance.' },
];

export default function MtnDataScreen({ user, onBack, themeMode = 'dark', onOpenDeposit, onSuccess }) {
  ...
  async function tryBiometricAuth() {
    try {
      ...
      if (res.success) {
        ...
        const payload = { success: true, provider: 'mtn', phone, selectedPackage, amount: getPayableAmount(), voucherApplied, timestamp: Date.now() };
  ...
  function verifyNumber() {
    ...
    // normalize: if starts with 234 -> convert to 0...
    let normal = raw;
  ...
  function getAmountForPackage(pkg) {
    ...
    // handle simple pill ids like '1GB'
    ...
    if (title.includes('1gb') || title.includes('1.2gb')) return 250;
  ...
  function startPurchase() {
    ...
    loop.start();
    setTimeout(() => {
      // stop looping and proceed to auth
  ...
  React.useEffect(() => {
    if (authVisible) {
      // focus hidden TextInput so keyboard opens
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: palette.background, paddingTop: safeTop }]}> 
      ...
      {/* balance card like main Data screen */}
      <View style={[styles.balanceCard, { backgroundColor: palette.surface }]}> 
        ...
        <View style={styles.delRateRow}>
          ...
          <Text style={[styles.delPct, { color: palette.text }]}>94.00%</Text>
        </View>
        <Text style={[styles.dialText, { color: palette.textMuted }]}>Dial *461*4# to check data balance</Text>
      </View>
      ...
      <Modal visible={authVisible} animationType="slide" transparent>
        <View style={styles.authOverlay}>
          <View style={[styles.authSheet, { backgroundColor: palette.surface }]}> 
            ...
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Product</Text><Text style={[styles.authValue, { color: palette.text }]}>{selectedPackage?.title || 'MTN Data Bundle'}</Text></View>
