...
export default function NinemobileAirtimeScreen({ user, onBack, themeMode = 'dark', onOpenDeposit, onSuccess }) {
  ...
  const NINE_PREFIXES = ['0809','0817','0818','0909'];
  ...
  function handlePay() {
    ...
    const payload = { success: true, provider: 'ninemobile', phone, amount: payable, voucherApplied, timestamp: Date.now() };
  ...
  React.useEffect(() => {
    ...
    if (normal.length >= 10) {
      const isNine = NINE_PREFIXES.some((pre) => normal.startsWith(pre));
      setVerified(isNine);
      if (!isNine) setVerifyError('Not a 9mobile number');
      else setVerifyError('');
    } else {
      setVerified(false);
      setVerifyError('');
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: palette.background, paddingTop: safeTop }]}> 
      ...
      <View style={[styles.balanceCard, { backgroundColor: palette.surface }]}> 
        ...
        <View style={styles.delRateRow}>
          ...
          <Text style={[styles.delLabel, { color: palette.textMuted }]}>9mobile Delivery Rate Nationwide</Text>
              ...
              <View style={[styles.card, { backgroundColor: palette.surface }]}> 
                ...
                {verified && (
                  <View style={styles.verifiedRow}>
                    ...
                    <Text style={[styles.verifiedText, { color: '#2ECC71' }]}> Verified 9mobile Number</Text>
                  </View>
                )}
      ...
      <Modal visible={authVisible} animationType="slide" transparent>
        <View style={styles.authOverlay}>
          <View style={[styles.authSheet, { backgroundColor: palette.surface }]}> 
            ...
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Product</Text><Text style={[styles.authValue, { color: palette.text }]}>{'9mobile Airtime'}</Text></View>
