...
const DUMMY_PACKAGES = [
  ...
  { id: 'p2', title: '1GB - 7 Days', subtitle: 'Dial *127*0# to check data balance' },
  { id: 'p3', title: '2GB - 30 Days', subtitle: 'Works for all apps' },
  { id: 'p4', title: '3GB - 30 Days', subtitle: 'Dial *127*0# to check data balance.' },
];

export default function GloDataScreen({ user, onBack, themeMode = 'dark', onOpenDeposit, onSuccess }) {
  ...
  async function tryBiometricAuth() {
    try {
      ...
      if (res.success) {
        ...
        const payload = { success: true, provider: 'glo', phone, selectedPackage, amount: getPayableAmount(), voucherApplied, timestamp: Date.now() };
  ...
  const GLO_PREFIXES = ['0805','0807','0815','0811','0705','0905'];
  ...
  function verifyNumber() {
    ...
    const isGlo = GLO_PREFIXES.some((pre) => normal.startsWith(pre));
    if (isGlo) {
      setVerified(true);
    ...
    return (
      <TouchableOpacity style={[styles.pkgRow, { backgroundColor: palette.surface }]} onPress={() => { setSelectedPackage(item); setPackagesVisible(false); }}>
        <View style={styles.pkgIconWrap}>
          <Image source={require('../../public/Glo-logo.png')} style={styles.pkgIcon} />
        </View>
  ...
  function getAmountForPackage(pkg) {
    ...
    if (title.includes('500mb')) return 150;
    if (title.includes('1gb') ) return 250;
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: palette.background, paddingTop: safeTop }]}> 
      ...
      <View style={[styles.balanceCard, { backgroundColor: palette.surface }]}> 
        ...
        <Image source={require('../../public/Glo-logo.png')} style={styles.mtnLogo} />
        <View style={styles.delRateRow}>
          <View style={[styles.delBarBackground, { backgroundColor: barBgColor }]}> 
            <View style={[styles.delBarFill, { width: '92%', backgroundColor: '#2ECC71' }]} />
          </View>
          <Text style={[styles.delLabel, { color: palette.textMuted }]}>Glo Delivery Rate Nationwide</Text>
          <Text style={[styles.delPct, { color: palette.text }]}>92.00%</Text>
        </View>
        <Text style={[styles.dialText, { color: palette.textMuted }]}>Dial *127*0# to check data balance</Text>
      </View>
      ...
      <Modal visible={authVisible} animationType="slide" transparent>
        <View style={styles.authOverlay}>
          <View style={[styles.authSheet, { backgroundColor: palette.surface }]}> 
            ...
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Product</Text><Text style={[styles.authValue, { color: palette.text }]}>{selectedPackage?.title || 'Glo Data Bundle'}</Text></View>
