...
export default function GloAirtimeScreen({ user, onBack, themeMode = 'dark', onOpenDeposit, onSuccess }) {
  ...
  function handlePay() {
    ...
    const payload = { success: true, provider: 'glo', phone, amount: payable, voucherApplied, timestamp: Date.now() };
  ...
  React.useEffect(() => {
    ...
    if (normal.length >= 10) {
      const isGlo = GLO_PREFIXES.some((pre) => normal.startsWith(pre));
      setVerified(isGlo);
      if (!isGlo) setVerifyError('Not a Glo number');
      else setVerifyError('');
    } else {
      setVerified(false);
      setVerifyError('');
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: palette.background, paddingTop: safeTop }]}> 
      ...
      <View style={[styles.balanceCard, { backgroundColor: palette.surface }]}> 
        ...
        <Text style={[styles.dialText, { color: palette.textMuted }]}>Dial *127*0# to check airtime balance</Text>
      </View>
              ...
              <View style={[styles.card, { backgroundColor: palette.surface }]}> 
                ...
                {verified && (
                  <View style={styles.verifiedRow}>
                    ...
                    <Text style={[styles.verifiedText, { color: '#2ECC71' }]}> Verified Glo Number</Text>
                  </View>
                )}
      ...
      <Modal visible={authVisible} animationType="slide" transparent>
        <View style={styles.authOverlay}>
          <View style={[styles.authSheet, { backgroundColor: palette.surface }]}> 
            ...
            <View style={styles.authRow}><Text style={[styles.authLabel, { color: palette.textMuted }]}>Product</Text><Text style={[styles.authValue, { color: palette.text }]}>{'Glo Airtime'}</Text></View>
